...
* **Shield drop** (red alien) — drops only when no shield is active; on bottom contact, the shield activates for `ShieldSettings.DURATION_MS`. The CRT vignette is a constant white wash with a blue overlay smoothly cosine-pulsing in and out on top (period `ShieldSettings.PULSE_PERIOD_MS`, peak alpha `PULSE_BLUE_ALPHA_MAX`). During the final `ShieldSettings.WARNING_MS` ms the smooth pulse switches to a rapid binary blue strobe driven by `WARNING_FLASH_INTERVAL`. Bottom-of-screen alien hits become shielded kills (explosion + score, no heart cost) for the entire shield window.
